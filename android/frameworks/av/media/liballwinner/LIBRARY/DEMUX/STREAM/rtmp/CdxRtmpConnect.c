   	...
   	while (1)
    	{
			...
			nBytes = sizeof(sb->sbBuf) - sb->sbSize
					- (sb->sbStart - sb->sbBuf);
			readLen = recv(sb->sbSocket, sb->sbStart + sb->sbSize, nBytes, 0); //* 接受数据存入socket缓存
			if (readLen != -1)
			{
				sb->sbSize += readLen;
			}
...
*/
char *aw_amf_encode_named_string(char *pOutput, char *pOutend, aw_rtmp_aval_t *strName, aw_rtmp_aval_t *strValue)
{
...
#define AW_RTMP_PACKET_TYPE_SERVER_BW          0x05      /* the spec present is Windows Acknowledge message(?)  */
...
int aw_rtmp_connect0(aw_rtmp_t *r, struct sockaddr * service)
{
    ...
    {
        if(connect(r->sb.sbSocket, service, sizeof(struct sockaddr)) != 0) //*connect将socket连接到服务端
	    {
...
//        	fcntl( r->sb.sbSocket, F_SETFL, fcntl(r->sb.sbSocket, F_GETFL, 0) & (~O_NONBLOCK) );
    ...
    else
    {
        ...
        //* set timeout
    SET_RCVTIMEO(tv, r->link.timeout);
    setsockopt(r->sb.sbSocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv, sizeof(tv));  //*设置套接字属性
    setsockopt(r->sb.sbSocket, IPPROTO_TCP, TCP_NODELAY, (char *) &on, sizeof(on));
...
static int aw_send_connect_packet(aw_rtmp_t *r, aw_rtmp_packet_t *cp)
{
    ...
    enc = aw_amf_encode_number(enc, pend, ++r->numInvokes);        /* connect transcation id (always set to 1)*/
    *enc++ = RTMP_AMF_OBJECT;  /* command object(AMF0 type is 0x03) */
    ...
    if(!(r->link.protocol & CDX_RTMP_FEATURE_WRITE))
    {
        ...
        enc = aw_amf_encode_named_number(enc, pend, (aw_rtmp_aval_t*)(&r->rtmpParam.av_audioCodecs), r->fAudioCodecs);
        if(!enc)
	    {
            return 0;
        }
        enc = aw_amf_encode_named_number(enc, pend, (aw_rtmp_aval_t*)(&r->rtmpParam.av_videoCodecs), r->fVideoCodecs);
        if(!enc)
	    {
            return 0;
        }
    ...
    {	
        ...
        enc = aw_amf_encode_named_number(enc, pend, (aw_rtmp_aval_t*)(&r->rtmpParam.av_objectEncoding), r->fEncoding);
        if(!enc)
	    {
            return 0;
        }
...
int aw_rtmp_read_packet(aw_rtmp_t*r, aw_rtmp_packet_t *packet)
{
    ...
    if(nSize >= 3)
    {
        ...
        if(packet->nTimeStamp == 0xffffff)  /*if the time stamp is the max number, need the extend time stamp*/
	    {
...
//        didAlloc = 1;
    ...
    nToRead = packet->nBodySize - packet->nBytesRead; /*the data size of same message in different chunk */
    ...
    if(packet->nBytesRead == packet->nBodySize)  
    {
	    ...
	    {
            packet->nTimeStamp += r->channelTimestamp[packet->nChannel];  /* timestamps seem to be always relative!! */
        }
        ...
        r->vecChannelsIn[packet->nChannel]->nHasAbsTimestamp = 0;	/* can only be false if we reuse header */
    }
    else
    {
...
{
    int nOriginalSize = nSize;
    int bError = 0;		/* if there is an error while decoding - try to at least find the end mark RTMP_AMF_OBJECT_END */
...
int aw_rtmp_client_packet(aw_rtmp_t*r, aw_rtmp_packet_t *packet)
{
    ...
    switch(packet->mPacketType)  /*message(AMF) type id*/
    {
        ...
        case AW_RTMP_PACKET_TYPE_CONTROL:     /*use control message*/
            ...
            break;
        case AW_RTMP_PACKET_TYPE_SERVER_BW:     /* spec descrip it window acknownledge size, but code is not*/
            /* server bw */
        ...
        case AW_RTMP_PACKET_TYPE_FLASH_VIDEO:  /* Aggregate Message */
        {
	        ...
	        {
	            unsigned int dataSize = aw_amf_decode_int24(packet->pBody + pos + 1);	/* size without header (11) and prevTagSize (4) */

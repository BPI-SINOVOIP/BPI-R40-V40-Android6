...
#define CDX_RTMP_PROTOCOL_RTMFP     CDX_RTMP_FEATURE_MFP
 ...
 {
    int      nHeaderSize;         /* chunk header size(basic header, message header, extend time stamp)*/
 ...
 {    
    ...
    int                   nInfoField2;                  /* message stream id ,last 4 bytes in a long header */
...
typedef struct RTMP
{
    ...
    int                  streamBufSize;     /* cache size, set in RtmpStreamOPen(), default value is MAX_STREAM_BUF_SIZE */

    av_rtmp_param_t      rtmpParam;
    int                  inChunkSize;       /* read chunk size (128 bytes default) (server to client)*/
    int                  outChunkSize;      /* send chunk size (128 bytes default) (client send command to server)*/

    int                  nBWCheckCounter;   /* used for calculate time stamp of send message to server */
    int                  nBytesIn;          /* recieve data size from server,used for send Acknownledge size command to server */
    ...
    int                  pausing;		      /* 1: pause,  2: stream eof, 3: unkown(User control Message Event=31, unkown) */
    ...
    int                  channelTimestamp[RTMP_CHANNELS];  /* absolute timestamp of every packet with different stream id */
    ...
    double               fDuration;        /* the "duration" in metadata ,duration of stream in seconds */

    int                  mMsgCounter;       /*(not used) http post header, RTMPT stuff (not used now)*/

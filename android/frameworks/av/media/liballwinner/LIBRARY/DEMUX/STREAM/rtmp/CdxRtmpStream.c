...
} options[19] = 
{
  ...
  { AVCSTR("flashver"),  OFFSET(link.flashVer),      STRING,  0,"Flash version string (default " DEFAULT_VERSTR ")" },
  { AVCSTR("conn"),      OFFSET(link.extras),        CONN, 0,"Append arbitrary AMF data to Connect message" },
  { AVCSTR("playpath"),  OFFSET(link.playpath),      STRING,  0,"Path of target media on server" },
  { AVCSTR("playlist"),  OFFSET(link.lFlags),        BOOL, RTMP_LF_PLST, "Set playlist before call play command" },
  { AVCSTR("live"),      OFFSET(link.lFlags),        BOOL, RTMP_LF_LIVE, "Stream is live, unseekable" },
  ...
  { AVCSTR("swfVfy"),    OFFSET(link.lFlags),        BOOL, RTMP_LF_SWFV, "Perform swf Verification" },
  { AVCSTR("swfAge"),    OFFSET(link.swfAge),        INT,  0,"Number of day to use cached SWF hash" },
...
new_read_packet:
    ...
    {   
        ...
        /****Return CDX_RTMP_READ_COMPLETE if this was completed nicely with invoke message Play.Stop or Play.Complete*/
        ...
        if(packet.mPacketType == RTMP_PACKET_TYPE_VIDEO && nPacketLen <= 5)   /*video chunk do not have data*/
        {   

            goto new_read_packet;
        }
        if(packet.mPacketType == RTMP_PACKET_TYPE_AUDIO && nPacketLen <= 1)  /*audio chunk do not have data*/
        {   
...
int aw_rtmp_stream_read(aw_rtmp_t* rtmp, char* buf, int size)
{
    ...
    {   
        ...
        {
            nRead = aw_rtmp_read_one_packet(rtmp, buf, size);  /*读取网络流数据到rtmp客户端buffer*/
            if(nRead <= 0)
            {   

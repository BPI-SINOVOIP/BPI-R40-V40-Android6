...
static int MmstInterpHeader (aw_mms_inf_t* mmsStreamInf, unsigned char *header, int header_len)
{
    ...
    while (i<header_len) 
    {
        ...
        if((0x6cce6200aa00d9a6ULL == guid_1) && (0x11cf668e75b22630ULL == guid_2) )  /* ASF_HEADER_Object*/
        {
            logv("header object");
        } 
        else if ((0x6cce6200aa00d9a6ULL == guid_1) && (0x11cf668e75b22636ULL == guid_2)) /*ASF_Data_Object*/
        {
            logv("data object");
        }
        else if((0x6553200cc000e48eULL == guid_1) && (0x11cfa9478cabdca1ULL == guid_2)) /*ASF_File_Properties_Object*/
        {
            packetLength = Get32(header, i+92-24);  //*  Minimum Data Packet Size in ASF_File_Properties_Object
            logv("file property object");
        } 
        else if((0x6553200cc000e68eULL == guid_1) && (0x11cfa9b7b7dc0791ULL == guid_2)) /*ASF_Stream_Properties_Object*/
        {
...
int MmstStreamingStart(char *uri, aw_mms_inf_t* mmsStreamInf)
{
 ...
 /*
   ...
   */
    ...
    //* fs is server socket
    //* void MmstSendCommand(int* seqNum,int s, int command, unsigned int switches, unsigned int extra, int length, unsigned char *sendData)
    ret = MmstSendCommand(&mmsStreamInf->seqNum, fs, 1, 0, 0x0004000b, strlen(str)*2+2, (unsigned char*)sendData);
    if(ret < 0)
    {
  ...
  * Where:
  * [00 00] shows 0x61 0x00 (on the first 33 sent) or 0xff 0xff for ASF files, and with no ending sendData for WMV files.
  ...
  */
    ...
    sendData[20] = 0x04;
    MmstSendCommand(&mmsStreamInf->seqNum,fs, 0x07, 1, 0xFFFF | mmsStreamInf->streamIds[0] << 16, 24, (unsigned char*)sendData);
    len = MmsGetNetworkData(mmsStreamInf, fs,sendData, MMST_BUF_SIZE);
    if(len <= 0)
    {
        closesocket(fs);

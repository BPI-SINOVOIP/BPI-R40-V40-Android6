...
static cdx_int32 __EnvStreamRead(CdxStreamT *stream, cdx_void *buf, cdx_uint32 len)
{
    struct CdxEnvStreamImplS *impl;
...
static cdx_int32 __EnvStreamControl(CdxStreamT *stream, cdx_int32 cmd, cdx_void *param)
{
    struct CdxEnvStreamImplS *impl;
...
static cdx_int32 __EnvStreamSeek(CdxStreamT *stream, cdx_int64 offset, cdx_int32 whence)
{
    ...
    //CDX_LOGD("%s:%d, off=%lld, whence=%d, new=%d, ret=%d", __func__, __LINE__, offset, whence, new_pos, ret);
    return ret == 0 ? 0 : -1;
}
...
static cdx_int32 __EnvStreamGetMetaData(CdxStreamT *stream, const cdx_char *key, cdx_void **pVal)
{
    struct CdxEnvStreamImplS *impl;
...
static CdxStreamT *__EnvStreamCreate(CdxStreamT *innerstream)
{
    ...
    if (DRM_REVOCATION_IsRevocationSupported())
    {
        ...
        ChkDR( Drm_Revocation_SetBuffer(impl->drm_app_context, impl->drm_revocation_buffer, REVOCATION_BUFFER_SIZE) );
    }

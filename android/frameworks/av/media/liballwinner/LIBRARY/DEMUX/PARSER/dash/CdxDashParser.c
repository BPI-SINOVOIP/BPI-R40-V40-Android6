...
char* CdxUrlConcatenate(const char* pParentName, const char* pPathName)
{
	...
	if (pPathName[0] == '.') 
	{
		...
		for (i = 0; i< (int)strlen(pPathName) - 2; i++) 
		{
			...
			/*parent dir*/
			if ( (pPathName[i] == '.') && (pPathName[i+1] == '.') && ( (pPathName[i+2] == '\\') || (pPathName[i+2] == '/') ))
			{
				nPathSepCount ++;
				i+=2;
...
int CdxDashGetSegmentDuration(awRepresentation *rep, awAdaptionSet *set, awPeriod *period, awMpd* mpd,
					 int *nbSegments, long long int *maxSegDuration)
{
...
*/
/*************************************************************************/
static int CdxDashResolveDuration(awRepresentation *rep, awAdaptionSet *set, awPeriod *period, long long int *outDuration, int *outTimescale, long long int *outPtsOffset, awSegmentTimeline **outSegmentTimeline)
{
	int timescale = 0;
	...
	if (rep->segmentBase || set->segmentBase || period->segmentBase) 
	{
		*outDuration = period ? period->duration : 0;
		timescale = 0;
		if (rep->segmentBase && rep->segmentBase->presentationTimeOffset) ptsOffset = rep->segmentBase->presentationTimeOffset;
		if (rep->segmentBase && rep->segmentBase->timescale) timescale = rep->segmentBase->timescale;
		if (!ptsOffset && set->segmentBase && set->segmentBase->presentationTimeOffset) ptsOffset = set->segmentBase->presentationTimeOffset;
		if (!timescale && set->segmentBase && set->segmentBase->timescale) timescale = set->segmentBase->timescale;
		if (!ptsOffset && period->segmentBase && period->segmentBase->presentationTimeOffset) ptsOffset = period->segmentBase->presentationTimeOffset;
	...
	mbaseRep = rep->segmentList ? (awMultipleSegmentBase *) rep->segmentList : (awMultipleSegmentBase *) rep->segmentTemplate;
	mbaseSet = set->segmentList ? (awMultipleSegmentBase *)set->segmentList : (awMultipleSegmentBase *)set->segmentTemplate;
	mbasePeriod = period->segmentList ? (awMultipleSegmentBase *)period->segmentList : (awMultipleSegmentBase *)period->segmentTemplate;
	...
	*outTimescale = timescale;
	...
	if (outPtsOffset) {
		...
		if (!ptsOffset && mbaseSet && mbaseSet->presentationTimeOffset) ptsOffset = mbaseSet->presentationTimeOffset;
		if (!ptsOffset && mbasePeriod && mbasePeriod->presentationTimeOffset) ptsOffset = mbasePeriod->presentationTimeOffset;
...
static int CdxDashResolveUrl(CdxDashParser* dashStream, awPeriod* period,awAdaptionSet* set, awRepresentation* rep,
					CDX_DASHURLResolveType resolveType, int itemIndex, char** outUrl, long long int *segmentDuration)
{
	char* url;
	awBaseURL* urlChild;
	...
	*segmentDuration = (resolveType==CDX_DASH_RESOLVE_URL_MEDIA) ? (int) ((double) (*segmentDuration) * 1000.0 / timescale) : 0;
	...
	if(rep->segmentList || set->segmentList || period->segmentList)
	{
		...
		if(period->segmentList)
		{
			if(period->segmentList->initializationSegment)    initUrl = period->segmentList->initializationSegment;
			if(period->segmentList->representationIndex)     indexUrl = period->segmentList->representationIndex;
			if(period->segmentList->segmentURL)               segments = period->segmentList->segmentURL;
			if(period->segmentList->startNumber != (unsigned int) -1) startNumber = period->segmentList->startNumber;
			if(period->segmentList->segmentTimeline) timeLine = period->segmentList->segmentTimeline;
		}

		if(set->segmentList)
		{
			if(set->segmentList->initializationSegment)    initUrl = period->segmentList->initializationSegment;
			if(set->segmentList->representationIndex)     indexUrl = period->segmentList->representationIndex;
			if(set->segmentList->segmentURL)               segments = period->segmentList->segmentURL;
			if(set->segmentList->startNumber != (unsigned int) -1) startNumber = period->segmentList->startNumber;
			if(set->segmentList->segmentTimeline) timeLine = period->segmentList->segmentTimeline;
		}

		if(rep->segmentList)
		{
			if(rep->segmentList->initializationSegment)    initUrl = period->segmentList->initializationSegment;
			if(rep->segmentList->representationIndex)     indexUrl = period->segmentList->representationIndex;
			if(rep->segmentList->segmentURL)               segments = period->segmentList->segmentURL;
			if(rep->segmentList->startNumber != (unsigned int) -1) startNumber = period->segmentList->startNumber;
			if(rep->segmentList->segmentTimeline) timeLine = period->segmentList->segmentTimeline;
		}
	...
	if(period->segmentTemplate)
	{
		if(period->segmentTemplate->initialization)    initTemplate = period->segmentTemplate->initialization;
		if(period->segmentTemplate->index)     indexTemplate = period->segmentTemplate->index;
		if(period->segmentTemplate->media)     mediaUrl= period->segmentTemplate->media;
		if(period->segmentTemplate->startNumber != (unsigned int) -1) startNumber = period->segmentTemplate->startNumber;
		if(period->segmentTemplate->segmentTimeline) timeLine = period->segmentTemplate->segmentTimeline;
	}

	if(set->segmentTemplate)
	{
		if(set->segmentTemplate->initialization)    initTemplate = set->segmentTemplate->initialization;
		if(set->segmentTemplate->index)     indexTemplate = set->segmentTemplate->index;
		if(set->segmentTemplate->media)     mediaUrl= set->segmentTemplate->media;
		if(set->segmentTemplate->startNumber != (unsigned int) -1) startNumber = set->segmentTemplate->startNumber;
		if(set->segmentTemplate->segmentTimeline) timeLine = set->segmentTemplate->segmentTimeline;
	}

	if(rep->segmentTemplate)
	{
		if(rep->segmentTemplate->initialization)    initTemplate = rep->segmentTemplate->initialization;
		if(rep->segmentTemplate->index)     indexTemplate = rep->segmentTemplate->index;
		if(rep->segmentTemplate->media)     mediaUrl= rep->segmentTemplate->media;
		if(rep->segmentTemplate->startNumber != (unsigned int) -1) startNumber = rep->segmentTemplate->startNumber;
		if(rep->segmentTemplate->segmentTimeline) timeLine = rep->segmentTemplate->segmentTimeline;
	}
...
int CdxDashInitSegmentUrl(CdxDashParser* dashStream, int type)
{
	...
	//if(v_rep)
	{
		ret = CdxDashResolveUrl(dashStream, period, v_set, v_rep, CDX_DASH_RESOLVE_URL_INIT, 0, &initUrl, &segmentDuration);
		if(ret < 0)
		{
...
int CdxDashGetSegmentUrl(CdxDashParser* dashStream, awPeriod* period, awAdaptionSet* set, awRepresentation* rep, int activeSegmentIndex)
{
	...
	ret = CdxDashResolveUrl(dashStream, period, set, rep, CDX_DASH_RESOLVE_URL_MEDIA, activeSegmentIndex, &segmentUrl, &segmentDuration);
	if(ret)
	{
		CDX_LOGE("resolve init segment url error!\n");
		if(segmentUrl)
...
int CdxDashSeekToTime(CdxDashParser* impl, cdx_int64 timeMs, int type)
{
	...
	ret = CdxDashGetSegmentIndex(impl->mediaStream[type].activeRepSetIndex, set, period, mpd, timeMs, &segmentDuration);
	if(ret < 0)
	{

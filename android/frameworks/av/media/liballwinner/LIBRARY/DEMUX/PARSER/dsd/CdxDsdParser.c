...
cdx_bool read_header(dsdiff_header *pHead, dsdfile *file) {
  ...
  pHead->size = GUINT64_FROM_BE(*((cdx_uint64*)pPtr)); pPtr+=8;
  memcpy(pHead->prop, pPtr, 4);
  if (!DSD_MATCH(pHead->prop, "DSD ")) return CDX_FALSE;
  return CDX_TRUE;
}
...
cdx_bool parse_prop_chunk(dsdfile *pFile, dsdiff_chunk_header *head) {
  ...
  if ((size_t)dsd_read_raw(propType, sizeof(propType), pFile) < sizeof(propType)) return CDX_FALSE;
  ...
  while (pFile->offset < stop) {
    ...
    if (DSD_MATCH(mProp_head.id, "FS  ")) {
      ...
      if ((size_t)dsd_read_raw(&uSample_frequency, sizeof(uSample_frequency), pFile)< sizeof(uSample_frequency)) return CDX_FALSE;
      pFile->sampling_frequency = GUINT32_FROM_BE(uSample_frequency);
    } else if (DSD_MATCH(mProp_head.id, "CHNL")) {
      cdx_uint16 uNum_channels;
      if ((size_t)dsd_read_raw(&uNum_channels, sizeof(uNum_channels), pFile) < sizeof(uNum_channels) ||
	  !dsd_seek(pFile, mProp_head.size - sizeof(uNum_channels), SEEK_CUR))return CDX_FALSE;
...
cdx_bool read_fmt_chunk(fmt_chunk *pDsf_fmt, dsdfile *file) {
  ...
  cdx_uint8 *pPtr=buffer;
  if ((size_t)dsd_read_raw(buffer, sizeof(buffer), file) < sizeof(buffer)) return CDX_FALSE;
  memcpy(pDsf_fmt->header, pPtr, 4); pPtr+=4;
  if (!DSD_MATCH(pDsf_fmt->header, "fmt ")) return CDX_FALSE;
  pDsf_fmt->size_of_chunk = GUINT64_FROM_LE(*((cdx_uint64*)pPtr)); pPtr+=8;
  if (pDsf_fmt->size_of_chunk != 52) return CDX_FALSE;
  pDsf_fmt->format_version = GUINT32_FROM_LE(*((cdx_uint32*)pPtr)); pPtr+=4;
  pDsf_fmt->format_id = GUINT32_FROM_LE(*((cdx_uint32*)pPtr)); pPtr+=4;
  pDsf_fmt->channel_type = GUINT32_FROM_LE(*((cdx_uint32*)pPtr)); pPtr+=4;
  pDsf_fmt->channel_num = GUINT32_FROM_LE(*((cdx_uint32*)pPtr)); pPtr+=4;
  pDsf_fmt->sampling_frequency = GUINT32_FROM_LE(*((cdx_uint32*)pPtr)); pPtr+=4;
  pDsf_fmt->bits_per_sample = GUINT32_FROM_LE(*((cdx_uint32*)pPtr)); pPtr+=4;
  pDsf_fmt->sample_count = GUINT64_FROM_LE(*((cdx_uint64*)pPtr)); pPtr+=8;
  pDsf_fmt->block_size_per_channel = GUINT32_FROM_LE(*((cdx_uint32*)pPtr)); pPtr+=4;
...
cdx_bool read_data_header(data_header *pDsf_datahead, dsdfile *file) {
  ...
  if ((size_t)dsd_read_raw(buffer, sizeof(buffer), file) < sizeof(buffer)) return CDX_FALSE;
  memcpy(pDsf_datahead->header, pPtr, 4); pPtr+=4;
  if (!DSD_MATCH(pDsf_datahead->header, "data")) return CDX_FALSE;
...
cdx_int32 aw_dsf_set_pos(dsdfile *pFile, cdx_uint32 mseconds, cdx_uint64 *offset) {
  ...
  uSkip_blocks = (cdx_uint64) pFile->sampling_frequency * mseconds / (8000 * pFile->dsf.block_size_per_channel);
...
static cdx_bool aw_dsdiff_init(dsdfile *pFile) {
  ...
  if (!read_header(&mHead, pFile)) return CDX_FALSE;
  ...
  while (read_chunk_header(&mChunk_head, pFile)) {
	...
	return CDX_FALSE;
      ...
      if (!parse_prop_chunk(pFile, &mChunk_head)) return CDX_FALSE;
    } else if (DSD_MATCH(mChunk_head.id, "DSD ")) {
      if (pFile->channel_num == 0) return CDX_FALSE;
...
static cdx_bool aw_dsf_init(dsdfile *pFile) {  
  ...
  if (!read_dsd_chunk_data(&mDsf_head, pFile)) return CDX_FALSE;
  
  if (!read_fmt_chunk(&mDsf_fmt, pFile)) return CDX_FALSE;
...
static cdx_bool aw_dsd_open(dsdfile *pFile) {
  ...
  if((size_t)dsd_read_raw(header_id,sizeof(header_id),pFile)<sizeof(header_id)) return CDX_FALSE;
  ...
  pFile->buffer.data = (cdx_uint8 *)malloc(sizeof(cdx_uint8) * pFile->buffer.max_bytes_per_ch * pFile->channel_num);
  return CDX_TRUE;
}

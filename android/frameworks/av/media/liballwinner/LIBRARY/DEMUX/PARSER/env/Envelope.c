...
{
    DRM_RESULT                dr                   = DRM_SUCCESS;
    DRM_APP_CONTEXT_INTERNAL *poAppContextInternal = DRM_REINTERPRET_CAST( DRM_APP_CONTEXT_INTERNAL, f_poAppContext );
    DRM_DWORD                 cbRead               = 0;
    ...
    */
    /*ChkBOOL( Oem_File_SetFilePointer( f_pEnvFile->pEnvFileHandle,
                                      ...
                                      OEM_FILE_CURRENT,
                                      NULL ), DRM_E_FILE_SEEK_ERROR );*/
    ChkBOOL((CdxStreamSeek(f_pEnvFile->pEnvFileHandle, f_pEnvFile->oEnvHeader.cbOriginalFilename, SEEK_CUR) == 0), DRM_E_FILE_SEEK_ERROR);
    ...
    */
    ...
    ChkBOOL((CdxStreamRead(f_pEnvFile->pEnvFileHandle,
                             poAppContextInternal->pbDRMHeaderData,
                             f_pEnvFile->oEnvHeader.cbDrmHeaderLen) == (cdx_int32)f_pEnvFile->oEnvHeader.cbDrmHeaderLen),
                             DRM_E_FILE_READ_ERROR );
    ...
    */
    if ( f_pEnvFile->dwFileDataStart > f_pEnvFile->oEnvHeader.cbHeaderSize )
    {
        ...
        ChkBOOL((CdxStreamSeek(f_pEnvFile->pEnvFileHandle, lFileStart, SEEK_SET) == 0), DRM_E_FILE_SEEK_ERROR);
    ...
    __in DRM_DECRYPT_CONTEXT        *f_pDecrypt )
{
    ...
    ChkDR( DRM_SECURECORE_DuplicateDecryptContext( &f_pEnvFile->oDecrypt, DRM_REINTERPRET_CAST( DRM_CIPHER_CONTEXT, f_pDecrypt ) ) );
    ...
    */
    if ( f_pEnvFile->oDecrypt.eCipherType == eDRM_RC4_CIPHER )
    {
        ...
        ChkBOOL((dwOldSeekPointer = CdxStreamTell(f_pEnvFile->pEnvFileHandle)) != (DRM_DWORD)-1, DRM_E_FILE_SEEK_ERROR);
        ...
        /*ChkBOOL( Oem_File_SetFilePointer( f_pEnvFile->pEnvFileHandle,
                                          ...
                                          OEM_FILE_END,
                                          NULL ), DRM_E_FILE_SEEK_ERROR );*/
        ChkBOOL((CdxStreamSeek(f_pEnvFile->pEnvFileHandle, -DRM_CPHR_CB_FOR_INITDECRYPT, SEEK_END) == 0), DRM_E_FILE_SEEK_ERROR);
        ...
        /* Restore the seek pointer. */
        {
            ...
            /*ChkBOOL( Oem_File_SetFilePointer( f_pEnvFile->pEnvFileHandle,
                                              lOldSeekPointer,
                                              OEM_FILE_BEGIN, NULL ), DRM_E_FILE_SEEK_ERROR );*/
            ChkBOOL((CdxStreamSeek(f_pEnvFile->pEnvFileHandle, lOldSeekPointer, SEEK_SET) == 0), DRM_E_FILE_SEEK_ERROR);

        }
        ChkDR( Drm_Cocktail_InitDecrypt( DRM_REINTERPRET_CAST( DRM_DECRYPT_CONTEXT, &f_pEnvFile->oDecrypt ), rgbLast15, dwFileSize ) );
    }
    else
    {
    ...
    __out DRM_DWORD                  *f_pcbFileSize )
{
    ...
    /*ChkBOOL( Oem_File_GetSize( f_pEnvFile->pEnvFileHandle, f_pcbFileSize ), DRM_E_FILE_SEEK_ERROR );*/
    ...
    __out                                             DRM_DWORD                  *f_pcbBytesRead )
{
    ...
    *f_pcbBytesRead = 0;
    ...
    if ( f_pEnvFile->oDecrypt.eCipherType == eDRM_AES_COUNTER_CIPHER )
    {
        ...
        ChkBOOL((dwFilePosition = CdxStreamTell(f_pEnvFile->pEnvFileHandle)) != (DRM_DWORD)-1, DRM_E_FILE_SEEK_ERROR);
    ...
    {
        /* Decrypt it. */
        ChkDR( DRM_SECURECORE_DecryptContentLegacy( &f_pEnvFile->oDecrypt, pCtrContext, *f_pcbBytesRead, f_pbBuffer ) );
    }
    ...
    __out DRM_DWORD                  *f_pdwNewFilePointer )
{
    ...
    ChkBOOL((CdxStreamSeek(f_pEnvFile->pEnvFileHandle, f_lDistanceToMove, f_dwMoveMethod) == 0), DRM_E_FILE_SEEK_ERROR);
    ChkBOOL((*f_pdwNewFilePointer = CdxStreamTell(f_pEnvFile->pEnvFileHandle)) != (DRM_DWORD)-1, DRM_E_FILE_SEEK_ERROR);

...
/*
		if(impl->bBufferring)
		{
			if(StreamCacheDataEnough(impl->mCache))
			{
				logd("data enough");
				PlayerStart(impl->mPlayer);
				impl->bBufferring = 0;
			}
			else
			{
		...
		if(StreamCacheUnderflow(impl->mCache))
		{
			if(PlayerBufferUnderflow(impl->mPlayer))
            {
                logd("detect player data underflow, notify BUFFER_START.");
                impl->bBufferring = 1;

                PlayerPause(impl->mPlayer);
            }
...
//        logd("IptvStreamCacheGetSize(impl->mCache): %d", IptvStreamCacheGetSize(impl->mCache));
		if(PlayerBufferOverflow(impl->mPlayer) || IptvStreamCacheGetSize(impl->mCache) <= 0)
		{
			usleep(10*1000);
			continue;
		}
        ...
        //********************************
        node = IptvStreamCacheNextFrame(impl->mCache);
	    if(node == NULL)
	    {
		...
		IptvStreamCacheFlushOneFrame(impl->mCache);
	}
...
void* CTC_MediaProcessorImpl::WriteDataThread(void* pArg)
{
	...
	while(1)
	{
		...
		{			
			if(findWH == 0)
			{
				ret = ProbeVideoSpecificData(&videoStreamInfo, impl->mDataBuffer, impl->mValidDataSize, VIDEO_CODEC_FORMAT_H265, CDX_PARSER_TS);
				if(ret == PROBE_SPECIFIC_DATA_SUCCESS)
				{
					...
					logd("++++ find width(%d), height(%d)", videoStreamInfo.nWidth, videoStreamInfo.nHeight);
				}
			}

			nowTime = GetNowUs();
			if((impl->mValidDataSize <= 4*1024*1024) && (nowTime - impl->mStartCacheTime < 1500*1000))
			{
				usleep(10*1000);
				continue;
			}
			...
			{
				impl->mFirstStartPlay = 1;
				logd("startplay, impl->mValidDataSize: %d, diff: %lld", impl->mValidDataSize, nowTime - impl->mStartCacheTime);
			}
		}
		...
		//logd("ts parser read size: %d, validSize: %d, readPtr: %p, buf: %p, bufEnd: %p", 
		//	nSize, impl->mValidDataSize, impl->mDataReadPtr, impl->mDataBuffer, impl->mDataBuffer+impl->mDataBufferSize);


		if(impl->mTsStreamBufSize)
		{
			logd("++++++ impl->mTsStreamBufSize: %d", impl->mTsStreamBufSize);
			pBuffer = (unsigned char*)impl->mTsStreamBuf;
			nSize = impl->mTsStreamBufSize;
		...
		if(nSize == 0 && impl->mLeftBytes < 205 && bufEndFlag)
		{
			memcpy(impl->mTsStreamBuf, impl->mDataReadPtr, impl->mLeftBytes);
			memcpy((unsigned char*)impl->mTsStreamBuf+impl->mLeftBytes, impl->mDataBuffer, PACKET_SIZE-impl->mLeftBytes);
...
int CTC_MediaProcessorImpl::WriteData(unsigned char* pBufferIn, unsigned int nSizeIn)
{
...
#if USE_LIST_CACHE
	if(IptvStreamCacheOverflow(mCache))
	{
		logv("StreamCacheOverflow");
		return -1;
	}
	...
	node.pNext   = NULL;
	IptvStreamCacheAddOneFrame(mCache, &node);
	...
	{
		//logd("++++++++ StreamCacheOverflow, mValidDataSize: %d, nSizeIn: %d, mDataBufferSize: %d", mValidDataSize, nSizeIn, mDataBufferSize);
		return -1;
	}
...
//	logd("mNativeWindow '%p'", mNativeWindow.get());
	...
	if(mNativeWindow != NULL)
	{
#if (CONFIG_PRODUCT == OPTION_PRODUCT_TVBOX && CONFIG_CHIP == OPTION_CHIP_1680) //* for compile
...
JNIEnv* CTC_MediaProcessorImpl::getJNIEnv()
{
	if (mEnv == NULL) {
		...
		registerNatives = (typeof registerNatives)dlsym(libAndroidRuntimeDso, "Java_com_android_internal_util_WithFramework_registerNatives");
...
bool CTC_MediaProcessorImpl::SetVolume(int volume)
{
	...
	//int output = AudioSystem::getOutput(AUDIO_STREAM_MUSIC); // music_type
	//int ret = AudioSystem::setStreamVolume( AUDIO_STREAM_MUSIC, vol, output);
...
/*
	int output = AudioSystem::getOutput(AUDIO_STREAM_MUSIC); // music_type
	int ret = AudioSystem::getStreamVolume(AUDIO_STREAM_MUSIC, &vol, output);
	logd("PlayerGetVolume, ret = %d, vol = %f output: %d", ret, vol, output);
	return vol*100;
*/

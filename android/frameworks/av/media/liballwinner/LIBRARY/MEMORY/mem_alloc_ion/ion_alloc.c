...
unsigned long ion_alloc_alloc(int size)
{
	...
	if (g_alloc_context == NULL)
	{
		loge("ion_alloc do not opened, should call ion_alloc_open() before ion_alloc_alloc(size) \n");
		goto ALLOC_OUT;
	}
...
int ion_alloc_free(void * pbuf)
{
	...
	aw_mem_list_for_each_entry(tmp, &g_alloc_context->list, i_list)
	{
		if (tmp->vir == addr_vir)
		{
			...
			if (munmap((void *)(uintptr_t)(tmp->user_virt), tmp->size) < 0)
			{

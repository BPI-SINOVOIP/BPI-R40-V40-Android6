...
#if ((CONFIG_PRODUCT == OPTION_PRODUCT_PAD && CONFIG_CHIP == OPTION_CHIP_1639) || (LINUX_VERSION == LINUX_VERSION_3_10))
	#define USE_ION_DMA (1)
#else
	#define USE_ION_DMA (0)
#endif

//* on 1689 chip, not use the custom
#if(CONFIG_CHIP != OPTION_CHIP_1689)
	#define USE_ION_CUSTOM (1)
#else
	#define USE_ION_CUSTOM (0)
#endif	

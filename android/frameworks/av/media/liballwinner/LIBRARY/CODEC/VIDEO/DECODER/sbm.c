...
*   Lin-Beihong            2013.12.14                             Add methods.
**********************************************************************
*/
...
Sbm *SbmSecureosCreate(int nBufferSize)
{
    ...
    pSbmBuf = (char*)AdapterSecureMemAdapterAlloc(nBufferSize);    //*
    if(pSbmBuf == NULL)
    {
        loge("pSbmBuf == NULL.");
        return NULL;
    ...
    {
        loge("pSbm == NULL.");
        AdapterSecureMemAdapterFree((void*)pSbmBuf); //*
        return NULL;
    }
    memset(pSbm, 0, sizeof(Sbm));
    ...
    if(pSbm->frameFifo.pFrames == NULL)
    {
        loge("sbm->frameFifo.pFrames == NULL.");
        free(pSbm);
        AdapterSecureMemAdapterFree(pSbmBuf);
        return NULL;
    }
    memset(pSbm->frameFifo.pFrames, 0,  SBM_FRAME_FIFO_SIZE * sizeof(VideoStreamDataInfo));
    for(i = 0; i < SBM_FRAME_FIFO_SIZE; i++)
...
void SbmSecureosDestroy(Sbm *pSbm)
{
    if(pSbm != NULL)
    {
        ...
        if(pSbm->pStreamBuffer != NULL)
        {
            AdapterSecureMemAdapterFree((void*)pSbm->pStreamBuffer);
            pSbm->pStreamBuffer = NULL;
        }

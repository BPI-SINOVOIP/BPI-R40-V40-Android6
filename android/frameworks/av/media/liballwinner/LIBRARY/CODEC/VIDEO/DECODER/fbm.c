...
void FbmDebugPrintStatus(Fbm* pFbm)
{
	...
	{
		logd("i=%d, picture=%p, displed=%d, valid=%d, decode=%d, render=%d\n", i, &pFbm->pFrames[i].vpicture, pFbm->pFrames[i].bAlreadyDisplayed,  pFbm->pFrames[i].bInValidPictureQueue,  pFbm->pFrames[i].bUsedByDecoder,
				 pFbm->pFrames[i].bUsedByRender);
	}
}
...
Fbm* FbmCreateBuffer(FbmCreateInfo* pFbmCreateInfo, VideoFbmInfo* pFbmInfo)
{
...
#if 0
	int k = 0;
	for(k = 0;k<p->nMaxFrameNum;k++)
	{
		logd("** create, bNeedRelease(%d), i(%d)",pFrameNode[k].bNeedRelease,k);
	}
#endif
    ...
    for(i=0; i<p->nMaxFrameNum; i++)
    {
        ...
        if(nAlignStride != 0)
        {
            pFrameNode->vpicture.nWidth       = (nWidth+nAlignStride-1) &~ (nAlignStride-1);
            pFrameNode->vpicture.nHeight      = (nHeight+nAlignStride-1) &~ (nAlignStride-1);
        }
        else
        {
       ...
       pFrameNode->vpicture.bIsProgressive = pFbmCreateInfo->bProgressiveFlag;
        ...
        if(pFbmCreateInfo->bGpuBufValid==0 || pFbmCreateInfo->nBufferType==BUF_TYPE_ONLY_REFERENCE)
        {   
    ...
    if(i < p->nMaxFrameNum)
    {
        ...
        int j;
    	loge("memory alloc fail, only %d frames allocated, we need %d frames.", i, pFbmCreateInfo->nFrameNum);
        for(j = 0; j < i; j++)
    ...
    if((pFbmCreateInfo->bGpuBufValid==1) && (pFbmCreateInfo->nBufferType!=BUF_TYPE_ONLY_REFERENCE))
    {
    	...
    	else
    	{
    		...
    		if(nAlignStride != 0)
    		{
    			pFbmInfo->pFbmBufInfo.nBufWidth = (nWidth+nAlignStride-1) &~ (nAlignStride-1);
    			pFbmInfo->pFbmBufInfo.nBufHeight = (nHeight+nAlignStride-1) &~ (nAlignStride-1);
    		}
    		else
    		{
	...
	if((pFbmInfo->bIs3DStream==1)&&(pFbmCreateInfo->bGpuBufValid==1))
	{
		if(pFbmInfo->pFbmFirst == NULL)
		{
			...
			if(pFbmInfo->pFbmFirst != NULL)
			{
				...
				GetBufferSize(pFbmInfo->pFbmBufInfo.ePixelFormat, pFbmInfo->pFbmBufInfo.nBufWidth, pFbmInfo->pFbmBufInfo.nBufHeight,
						&nYBufferSize, &nCBufSize, &nYLineStride, &nCLineStride, pFbmInfo->pFbmBufInfo.nAlignValue);
...
void FbmDestroy(Fbm* pFbm)
{
    ...
    {
        if(pFbm->bThumbnailMode)    //* in thumbnail mode, we only allocate one picture buffer.
            FbmFreePictureBuffer(&pFbm->pFrames[0].vpicture);
        else
        {
            for(i=0; i<pFbm->nMaxFrameNum; i++)
...
VideoPicture* FbmRequestBuffer(Fbm* pFbm)
{
    ...
    if((pFbmInfo->nMinorYBufOffset!=0) &&(pFbm == pFbmInfo->pFbmSecond))
    {
...
//            abort();
        ...
        if(pFbmInfo->nMinorYBufOffset != 0)
        {
    ...
    {
       	MemAdapterFlushCache((void*)pFrameNode->vpicture.pData0, 
...
void FbmReturnBuffer(Fbm* pFbm, VideoPicture* pVPicture, int bValidPicture)
{
    ...
    logv("FbmReturnBuffer pVPicture=%p, bValidPicture=%d, id=%d", pVPicture, bValidPicture, pVPicture->nID);
...
void FbmShareBuffer(Fbm* pFbm, VideoPicture* pVPicture)
{
    ...
    if(index < 0 || index >= pFbm->nMaxFrameNum)
    {
        loge("FbmShareBuffer: the picture id is invalid, pVPicture=%p, pVPicture->nID=%d, pFbm->nMaxFrameNum=%d",
    ...
    if(index < 0 || index >= pFbm->nMaxFrameNum)
    {
    	if(index >= FRM_FORBID_USE_TAG)
    	{
    		 ...
    		 if(pFrameNode->bUsedByRender!=0 ||
    		    ...
    		    pFrameNode->bAlreadyDisplayed != 0)
    		 {
                 loge(" check index valid error. bUsedByRender: %d, bInValidPictureQueue: %d, bAlreadyDisplayed: %d ",
                         pFrameNode->bUsedByRender, pFrameNode->bInValidPictureQueue, pFrameNode->bAlreadyDisplayed);
    			 //abort();
                 pthread_mutex_unlock(&pFbm->mutex);
                 return -1;
    		 }
...
int FbmAllocatePictureBuffer(VideoPicture* pPicture, int* nAlignValue, int nWidth, int nHeight)
{
    ...
    switch(ePixelFormat)
    {
        ...
        case PIXEL_FORMAT_NV21:
            ...
            pMem = (char*)AdapterMemPalloc(nMemSizeY + nMemSizeC*2);
            if(pMem == NULL)
            {
                loge("memory alloc fail, require %d bytes for picture buffer.", nMemSizeY + nMemSizeC*2);
                return -1;
            }
        ...
        case PIXEL_FORMAT_BGRA:
            ...
            AdapterMemFlushCache(pMem, nMemSizeY);
    ...
    {
    	pPicture->phyYBufAddr = (size_addr)AdapterMemGetPhysicAddress(pPicture->pData0);
    }
    if(pPicture->pData1 != NULL)
    {
    	pPicture->phyCBufAddr = (size_addr)AdapterMemGetPhysicAddress(pPicture->pData1);
    }
    return 0;
}
...
VideoPicture* FbmSetFbmBufAddress(VideoFbmInfo* pFbmInfo, VideoPicture* pPicture, int bForbidUseFlag)
{
	...
	{     
		  pFrameNode->vpicture.phyCBufAddr = pFrameNode->vpicture.phyYBufAddr+2*pFbmInfo->nMinorYBufOffset;
		  pFrameNode->vpicture.pData1 = pFrameNode->vpicture.pData0+2*pFbmInfo->nMinorYBufOffset;
	}
...
VideoPicture* FbmReturnReleasePicture(VideoFbmInfo* pFbmInfo, VideoPicture* pVpicture, int bForbidUseFlag)
{
	...
	{
		pFrameNode = &pFbm->pFrames[index];
	    if((pFrameNode->bNeedRelease==1)&&(pFrameNode->vpicture.nID == FRM_RELEASE_TAG))
	    {

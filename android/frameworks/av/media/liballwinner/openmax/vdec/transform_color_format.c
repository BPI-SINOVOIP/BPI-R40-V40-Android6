...
static void map32x32_to_yuv_Y(unsigned char* pSrcY, unsigned char* pTarY, unsigned int uCodedWidth, unsigned int uCodedHeight)
{
...
static void map32x32_to_yuv_C(int nMode,unsigned char* pSrcC,unsigned char* pTarCb,unsigned char* pTarCr,unsigned int uCodedWidth,unsigned int uCodedHeight)
{
...
static void map32x32_to_yuv_C_422(int nMode,unsigned char* pSrcC,unsigned char* pTarCb,unsigned char* pTarCr,unsigned int uCodedWidth,unsigned int uCodedHeight) {
	;
}
...
//					}
					asm volatile (
							...
							"vst1.8         {d1}, [%[pDstAsm1]]!              \n\t"
							"vst1.8         {d3}, [%[pDstAsm1]]!              \n\t"
					         : [pDstAsm0] "+r" (pDstAsm0), [pDstAsm1] "+r" (pDstAsm1), [pSrcAsm] "+r" (pSrcAsm)
					         :  //[pSrcY] "r" (pSrcY)
					         : "cc", "memory", "d0", "d1", "d2", "d3", "d4", "d5", "d6", "d16", "d17", "d18", "d19", "d20", "d21", "d22", "d23", "d24", "d28", "d29", "d30", "d31"
					         );
				}
...
void TransformMB32ToYV12(VideoPicture* pPicture, OutPutBufferInfo* pOutPutBufferInfo)
{
...
#if 0
	cedarx_cache_op(mScalerPara.pAddr_in_y, mScalerPara.pAddr_in_y+pPicture->size_y, CEDARX_DCACHE_FLUSH);
	cedarx_cache_op(mScalerPara.pAddr_in_c, mScalerPara.pAddr_in_c+pPicture->size_u, CEDARX_DCACHE_FLUSH);
#endif
    ...
    //* use neon accelarator instruction to transform the pixel format, slow if buffer is not cached(DMA nMode).
...
void TransformYV12ToYUV420Soft(VideoPicture* pPicture, OutPutBufferInfo* pOutPutBufferInfo)
{
    ...
    dstPtr = (unsigned char*)pOutPutBufferInfo->pAddr;
    srcPtr = (unsigned char*)pPicture->pData0 + (pPicture->nWidth * pPicture->nTopOffset + pPicture->nLeftOffset);
    ...
    logv("c dst w: %d,  dst h: %d;;   src w: %d, src h: %d; top: %d, left: %d, b:%d, r:%d",
          ...
          pPicture->nTopOffset, pPicture->nLeftOffset, pPicture->nBottomOffset, pPicture->nRightOffset);
...
//    logd("omx transformtyv12, pPicture->display_width[%d], pPicture->display_height[%d], pPicture:size_y[%d],size_u[%d]",
//        pPicture->display_width, pPicture->display_height, pPicture->size_y, pPicture->size_u);
...

    {
    	...
    	cWidth = (pPicture->display_width/2 + 15) & ~15;    //equal to GPUFBStride/2. hw_decoder's uv is 16pixel align
    	cHeight = heightAlign;
    	for(i=0; i<cHeight; i++)
    	{

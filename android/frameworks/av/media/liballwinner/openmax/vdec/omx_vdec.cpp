	    ...
	    case OMX_IndexParamPortDefinition:
	    {
            ...
            if (((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nPortIndex == m_sInPortDefType.nPortIndex)
            {
				...
				if(mIsCyber)
			    {
			        ...
			        //* palloc fbm in InitializeVideoDecode, shorten the time of first frame decoding
			        m_videoConfig.bSupportPallocBufBeforeDecode = 1;
			        m_streamInfo.nWidth = m_sInPortDefType.format.video.nFrameWidth;
			        m_streamInfo.nHeight = m_sInPortDefType.format.video.nFrameHeight;
			        logd("input w: %d, m_streamInfo.nHeight: %d", m_streamInfo.nWidth, m_streamInfo.nHeight);
					...
					logd("++++++++ m_videoConfig.bDispErrorFrame: %d", m_videoConfig.bDispErrorFrame);
	        ...
	        else if (((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nPortIndex == m_sOutPortDefType.nPortIndex)
	        {
				...
				if(mIsCyber)
				{
		            m_sOutPortDefType.format.video.nFrameWidth  = (m_sOutPortDefType.format.video.nFrameWidth+31) & ~31;
		            m_sOutPortDefType.format.video.nFrameHeight = (m_sOutPortDefType.format.video.nFrameHeight+31) & ~31;
	            }
...
OMX_ERRORTYPE  omx_vdec::empty_this_buffer(OMX_IN OMX_HANDLETYPE pHComponent, OMX_IN OMX_BUFFERHEADERTYPE* pBufferHdr)
{
...
OMX_ERRORTYPE  omx_vdec::fill_this_buffer(OMX_IN OMX_HANDLETYPE pHComponent, OMX_IN OMX_BUFFERHEADERTYPE* pBufferHdr)
{
...
drain_picture:            
        	...
        	if(ValidPictureNum(pSelf->m_decoder,0))
        	{
					...
					|| (mCurVideoRect.nHeight != pSelf->mVideoRect.nHeight))
            	{
					...
					pSelf->m_Callbacks.EventHandler(&pSelf->mOmxCmp, pSelf->m_pAppData, OMX_EventPortSettingsChanged, 0x01/*pSelf->m_sOutPortDefType.nPortIndex*/, 0, NULL);
					continue;
            	}
            	...
            	else
            	{
                    ...
                    android::Rect bounds(pSelf->m_sOutPortDefType.format.video.nFrameWidth, pSelf->m_sOutPortDefType.format.video.nFrameHeight);

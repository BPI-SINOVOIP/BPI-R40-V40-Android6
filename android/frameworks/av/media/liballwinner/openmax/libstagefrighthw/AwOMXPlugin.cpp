      ...
      mGetRolesOfComponentHandleFunc(NULL)
{
    if (mOmxLibHandle != NULL)
    {
        ...
        mComponentNameEnumFunc         = (OmxComponentNameEnumFunc)dlsym(mOmxLibHandle, "OMX_ComponentNameEnum");
        ...
        mGetRolesOfComponentHandleFunc = (OmxGetRolesOfComponentFunc)dlsym(mOmxLibHandle, "OMX_GetRolesOfComponent");
...
OMX_ERRORTYPE AwOMXPlugin::makeComponentInstance(const char* pName, const OMX_CALLBACKTYPE* pCallbacks, OMX_PTR pAppData, OMX_COMPONENTTYPE** ppComponent)
{
	logv("step 1.");
    if (mOmxLibHandle == NULL)
    {
...
OMX_ERRORTYPE AwOMXPlugin::getRolesOfComponent(const char *pName, Vector<String8> *vRoles)
{
    ...
    OMX_U32 uNumRoles;
    OMX_ERRORTYPE eErr = (*mGetRolesOfComponentHandleFunc)(const_cast<OMX_STRING>(pName), &uNumRoles, NULL);
    ...
    if (uNumRoles > 0)
    {
        ...
        eErr = (*mGetRolesOfComponentHandleFunc)(const_cast<OMX_STRING>(pName), &uNumRoles2, ppArray);

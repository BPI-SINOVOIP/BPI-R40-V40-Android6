...
struct awStreamingSource : public RefBase
{
    ...
    virtual status_t dequeueAccessUnit(bool audio, sp<ABuffer> *accessUnit) {CEDARX_UNUSE(audio);CEDARX_UNUSE(accessUnit);return OK;};
...
static cdx_int32 PrepareThread(CdxStreamT *stream)
{
	...
	streamingSource->mAwStreamingSource->copyAccessData(streamingSource->probeData.buf, ProbeDataLen);
...
static cdx_int32 GetCacheState(StreamingSource *streamingSource, struct StreamCacheStateS *cacheState)
{

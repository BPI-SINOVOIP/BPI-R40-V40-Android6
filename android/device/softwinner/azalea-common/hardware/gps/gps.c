...
#define AW_TELEMATICS
//for GPS begin
//#define  GPS_DEBUG
//#define	 GPS_DEBUG_TOKEN	/* print out NMEA tokens */
//for GPS end

#define  DFR(...)   ALOGW(__VA_ARGS__)
#ifdef GPS_DEBUG
#define  D(...)   ALOGW(__VA_ARGS__)
#define GPS_MSG(...)   ALOGW(__VA_ARGS__)
#else
#define  D(...)   ((void)0)
#define GPS_MSG(...)	((void)0)
#endif
...
void gps_state_lock_fix(GpsState *state) {
    int ret;
	D("gps_state_lock_fix in, state->fix_sem:%d\n",state->fix_sem.count);
    do {
	...
	D("gps_state_lock_fix out, state->fix_sem:%d\n",state->fix_sem.count);
...
void gps_state_unlock_fix(GpsState *state) {
	D("gps_state_unlock_fix in, state->fix_sem:%d\n",state->fix_sem.count);
	...
	D("gps_state_unlock_fix out, state->fix_sem:%d\n",state->fix_sem.count);
}
...
static int bHaveGpsTime = 0;		//add by Tommy
...
static int nmea_reader_get_timestamp(NmeaReader*  r, Token  tok, time_t *timestamp)
{
	...
	//*timestamp = mktime(&tm)- r->utc_diff;
//for GPS begin
...
static int setCurrentTimeMillis(int64_t millis)
{
    ...
    ALOGD("setCurrentTimeMillis: H--%x, L--%x\n", (int) (millis >> 32),(int)(millis&0xffffffff));
...
nmea_reader_parse( NmeaReader*  r )
{
	...
	if ( !memcmp(tok.p, "GNG", 3) ) //GPGSA,GPGGA,GPGSV
		bGetFormalNMEA = 1;
    // ignore first two characters.
    GPS_MSG("---get gps gng  data-");
    tok.p += 2;
	...
	GPS_MSG("---Get Gps data-----bHaveGpsTime:%d,started:%d,lastcmd:%d",bHaveGpsTime,started,lastcmd);
	...
	if((gps_state->first_fix == 1) && (bHaveGpsTime == 0)){
		int tmm = r->fix.timestamp / 1000L;
		if(tmm > 1449828000){
			setCurrentTimeMillis(r->fix.timestamp);
			bHaveGpsTime = 1;
		}
		else 
			ALOGE("get error gps time:%d\n", tmm);
	}
}
...
nmea_reader_addc( NmeaReader*  r, int  c )
{
    ...
    if (c == '\n') {
        ...
        r->pos = 0;
		if((bHaveGpsTime == 1) && started == 0)	//add by Tommy
			gps_sleep(gps_state);
   }
}
...
gps_state_done( GpsState*  s )
{
	...
	D("sem_destroy called,fix_sem:%d",s->fix_sem.count);
	g_gpscallback = 0;
...
void gps_sleep(GpsState *state)
{
	GPS_MSG("sleepping!!!,bHaveGpsTime = %d\n",bHaveGpsTime);
...
gps_state_thread( void*  arg )
{
    ...
    D("gps thread running,continue_thread = %d",continue_thread);
            ...
            if ((events[ne].events & EPOLLIN) != 0) {
                ...
                if (fd == control_fd)
                {
                    ...
                    if (cmd == CMD_QUIT) {
						...
						GPS_MSG("gps_state_thread-----CMD_QUIT");
                        goto Exit;
                    }
                    else if (cmd == CMD_START)
					{
						...
						GPS_MSG("gps_state_thread-----CMD_STOP");
                    }
                }
                else
                {
...
Exit:
	{
		...
		DFR("gps control thread destroyed,continue_thread = %d",continue_thread);
	}
    return;
}
...
gps_nmea_thread( void*  arg )
{
	...
	D("*#gps entered nmea thread,bOrionShutdown:%d,started:%d,sleep_lock:%d",bOrionShutdown,started,sleep_lock);  
	int versioncnt = 0;
   ...
   // now loop
    while (continue_thread)
	{
		...
		if (FD_ISSET(state->fd, &readfds))
		{
			...
			do{
			ret = read( state->fd, buf+readsum, sizeof(buf)- readsum ); 
			readsum += ret;
			}while(readsum < sizeof(buf));
			ret = readsum;
#else
...
gps_timer_thread( void*  arg )
{
  ...
  DFR("gps entered timer thread, started:%d",started);
...
#ifdef AW_TELEMATICS
         DFR("########### opening /dev/ttyS2**"); 
        state->fd = open( "/dev/ttyS2", O_RDWR | O_NOCTTY | O_NONBLOCK); 
#else
...
{
	if((s->fd != -1) && (bHaveGpsTime != 0))
	{
		D("%s", __FUNCTION__); 
		GPS_MSG("bHaveGpsTime = %d",bHaveGpsTime);
...
gps_state_init( GpsState*  state )
{
	...
	D("sem_init called,fix_sem:%d",state->fix_sem.count);

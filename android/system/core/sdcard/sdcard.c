...
static int mount_sdcardfs(const char *source_path, const char *dest_path,
        uid_t low_uid, gid_t low_gid, uid_t userid, gid_t upper_gid,
        mode_t mask, const char *label, const char *type, bool multi_user) {
    char opts[256];

    umount2(dest_path, MNT_DETACH);

    snprintf(opts, sizeof(opts),
            "low_uid=%d,low_gid=%d,userid=%d,gid=%d,mask=%o,label=%s,type=%s,lower_fs=%s%s",
            low_uid, low_gid, userid, upper_gid, mask, label, type,
            multi_user ? "ext4" : "fat", multi_user ? ",multi_user" : "");
...
static void run2(const char* source_path, const char* label, uid_t uid,
        gid_t gid, userid_t userid, bool multi_user, bool full_write) {
    char default_dest_path[PATH_MAX];
    char read_dest_path[PATH_MAX];
    char write_dest_path[PATH_MAX];
    snprintf(default_dest_path, PATH_MAX, "/mnt/runtime/default/%s", label);
    snprintf(read_dest_path, PATH_MAX, "/mnt/runtime/read/%s", label);
    snprintf(write_dest_path, PATH_MAX, "/mnt/runtime/write/%s", label);
    ...
    if (multi_user) {
        /* Multi-user storage is fully isolated per user, so "other"
         * permissions are completely masked off. */
        if (mount_sdcardfs(source_path, default_dest_path,
                    uid, gid, userid, AID_SDCARD_RW, 0006, label, "default", multi_user)
                || mount_sdcardfs(source_path, read_dest_path,
                    uid, gid, userid, AID_EVERYBODY, 0027, label, "read", multi_user)
                || mount_sdcardfs(source_path, write_dest_path,
                    uid, gid, userid, AID_EVERYBODY, full_write ? 0007 : 0027,
                    label, "write", multi_user)) {
            ERROR("failed to fuse_setup\n");
            exit(1);
        }
    } else {
        ...
        if (mount_sdcardfs(source_path, default_dest_path,
                    uid, gid, userid, AID_SDCARD_RW, 0006, label, "default", multi_user)
                || mount_sdcardfs(source_path, read_dest_path,
                    uid, gid, userid, AID_EVERYBODY, full_write ? 0027 : 0022,
                    label, "read", multi_user)
                || mount_sdcardfs(source_path, write_dest_path,
                    uid, gid, userid, AID_EVERYBODY, full_write ? 0007 : 0022,
                    label, "write", multi_user)) {
            ERROR("failed to fuse_setup\n");
            exit(1);
        }
    ...
    if (multi_user) {
        ...
        fs_prepare_dir(obb_path, 0775, uid, gid);
    }
...
#else
    run2(source_path, label, uid, gid, userid, multi_user, full_write);
#endif
